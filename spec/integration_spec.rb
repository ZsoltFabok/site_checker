...
describe "Integration" do
  ...
  describe "server based checking" do
    ...
    it "should visit the page" do
      ...
      @collector.local_pages.should eql([@test_url])
    ...
    it "should check the link to an external page" do
      ...
      @collector.check(@test_url, @root)
      @collector.remote_pages.should eql(["http://external.org/" ])
      @collector.problems.should be_empty
    end
    ...
    it "should report a problem if the external link is dead" do
      ...
      @collector.check(@test_url, @root)
      @collector.problems.should eql({@test_url => ["http://external.org/ (404)"]})
    end
    ...
    it "should report a problem if the image cannot be found" do
      ...
      @collector.check(@test_url, @root)
      @collector.problems.should eql({@test_url => ["http://external.org/a.png (404)"]})
    end
    ...
    it "should report a problem for a local page with absolute path" do
      ...
      @collector.check(@test_url, @root)
      @collector.problems.should eql({@test_url => ["#{@test_url}/another (absolute path)"]})
    end
    ...
    it "should report a problem for a local image with absolute path" do
      ...
      webmock(@test_url, 200, content)
      @collector.check(@test_url, @root)
      @collector.problems.should eql({@test_url => ["#{@test_url}/a.png (absolute path)"]})
    end
    ...
    it "should report an invalid internal anchor" do
      ...
      webmock(@test_url, 200, content)
      @collector.check(@test_url, @root)
      @collector.problems.should eql({@test_url => ["#goto (404 Not Found)"]})
    end
    ...
    it "should report a problem with a linked local page" do
      ...
      @collector.check(@test_url, @root)
      @collector.problems.should eql({@test_url => ["/one-level-down (404)"]})
    end
  end
  ...
  describe "file system based checking" do
    ...
    it "should find a referenced page" do
      ...
      @collector.check(fs_test_path, @root)
      @collector.local_pages.should eql([fs_test_path, "/one-level-down"])
      @collector.problems.should be_empty
    end
    ...
    it "should report a problem when the local page cannot be found" do
      ...
      filesystemmock("index.html", content)
      @collector.check(fs_test_path, @root)
      @collector.problems.should eql({fs_test_path => ["/one-level-down (404 Not Found)"]})
    end
    ...
    it "should use the local images" do
      ...
      @collector.check(fs_test_path, @root)
      @collector.local_images.should eql(["/a.png"])
      @collector.problems.should be_empty
    end
    ...
    it "should report a problem when the local image cannot be found" do
      ...
      filesystemmock("index.html", content)
      @collector.check(fs_test_path, @root)
      @collector.problems.should eql({fs_test_path => ["/a.png (404 Not Found)"]})
    end
